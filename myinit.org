...
The =(mapc #'package-install my-package-list)= line handles when I am first running my Emacs install on a new machine. It uses =package-list= to check for (and, if necessary, install) any packages into the default package-user-dir. Once those packages are downloaded and placed into that directory, they are autoloaded by Emacs.

Unfortunately, if you look at all the directories in =~/emacs.d/delpa=, you'll find only a tiny fraction of them have been added to the list. 
...
** server
This is so emacs starts as proper client
#+BEGIN_SRC emacs-lisp
;; (server-start)
...
I'm not sure what the following do. Maybe I should look 'em up?
...
So, I'm trying out code folding to give me that "Visual Studio" vibe when I'm previewing long bits of CloudFormation.
...
(set-face-background 'highlight-indentation-face "#f9f6f9")
(set-face-background 'highlight-indentation-current-column-face "#e3d3e3")
#+END_SRC

* ivy
First, a tweak to selection. I'm not too hip on what ivy is doing here with the multip matches, but this gets me so I can read stuff again in the minibuffer.
